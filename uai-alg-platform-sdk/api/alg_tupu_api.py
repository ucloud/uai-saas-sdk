...
class UAITupuCensorApi(UAIBaseDataStreamApi):

    param_pros = 'ProductNames'
    param_method = 'Method'
    param_image = 'Image'
    ...
    tupu_url = 'http://api.uai.ucloud.cn'

    def __init__(self, public_key, private_key, resource_id):
        super(UAITupuCensorApi, self).__init__(public_key, private_key, resource_id)

    def _check_params(self):
        super(UAITupuCensorApi, self)._check_params()
    ...
    def create_tupu_censor(self, productNames, method, url='', image=None):
        self.api_url = '/'.join([self.tupu_url, 'tupu/censor'])
        if len(productNames) == 0:
            return False, '{0} cannot be empty'.format(self.param_pros)
        method = method.lower()
        ...
        self.cmd_params[self.param_pros] = productNames
