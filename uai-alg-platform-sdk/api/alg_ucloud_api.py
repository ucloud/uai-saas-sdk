...
class UAIUCloudApi(UAIBaseDataStreamApi):
    ...
    ucloud_url = 'http://api.uai.ucloud.cn'

    def __init__(self, public_key, private_key, resource_id):
        super(UAIUCloudApi, self).__init__(public_key, private_key, resource_id)

    def _check_params(self):
        super(UAIUCloudApi, self)._check_params()
    ...
    def create_ucloud_mask(self, method, url='', image=None):
        self.api_url = '/'.join([self.ucloud_url, 'ucloud/mask'])
        method = method.lower()
    ...
    def create_ucloud_censor(self, scenes, method, url='', image=None):
        self.api_url = '/'.join([self.ucloud_url, 'ucloud/censor'])
        method = method.lower()
        if method == self.method_value_url:
            if url == '':
                return False, '{0} cannot be empty'.format(self.param_url)
