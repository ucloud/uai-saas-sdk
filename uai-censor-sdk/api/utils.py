...
def gen_async_video_censor_url():
    return string.join([DataStreamUrlPrefix, CensorVersion, VideoAsyncScan], "/")


def gen_query_async_video_censor_url():
    return string.join([DataStreamUrlPrefix, CensorVersion, VideoAsyncScan], "/")


def gen_async_video_toframe_url():
    return string.join([DataStreamUrlPrefix, CensorVersion, VideoToFrame], "/")


def gen_query_async_video_toframe_url():
    return string.join([DataStreamUrlPrefix, CensorVersion, VideoToFrame], "/")

def gen_signature(config_file, timestamp, url='', job_id=''):
