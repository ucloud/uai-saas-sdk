...
class ModifyUAICensorResourceOssOp(UAICensorBaseControllerOperation):

    def __init__(self, parser):
        super(ModifyUAICensorResourceOssOp, self).__init__(parser)

    def _add_oss_args(self, parser):
        args_parser = parser.add_argument_group(
            'Oss-Params', 'UAI Censor Modify Resource Oss Params'
        )
    ...
    def _add_args(self):
        super(ModifyUAICensorResourceOssOp, self)._add_args()
        self._add_oss_args(self.parser)

    def _parse_args(self, args):
        super(ModifyUAICensorResourceOssOp, self)._parse_args(args)
        self._parse_oss_args(args)

    def cmd_run(self, args):
        self._parse_args(args)
        caller = ModifyUAICensorResourceOssInfoApi(
